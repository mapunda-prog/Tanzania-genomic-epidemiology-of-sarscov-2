...
#summary statistics on age groups by number of cases
linelist_vars %>% 
  group_by(age_group, region) %>% 
  summarise(n = n())

#summary statistics on clades by number of cases
linelist_vars %>% 
...
# count the number of times each variant occurs and sort the top 30 variants

missense_variants_count <- missense_variants %>% 
  count(POS, HGVS_P_1LETTER) %>% arrange(desc(n)) %>% top_n(30)
missense_variants_count_l <- missense_variants %>% 
  count(POS, HGVS_P_1LETTER) %>% arrange(desc(n)) %>% top_n(100)

#creating a bar plot of the top 10 missense variants by SAMPLE
ggplot(data = missense_variants_count, aes(x = reorder(HGVS_P_1LETTER, n), y = n)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(
    title = "Top 10 missense variants in gene S",
    x = "HGVS_P_1LETTER",
    y = "Count"
  )

# select SAMPLE in missense_variants df where HGVS_P_1LETTER is equal to p.K417N
K417N <- missense_variants %>% filter(HGVS_P_1LETTER == "p.K417N")

G142D <- missense_variants %>% filter(HGVS_P_1LETTER == "p.G142D")
#creating a matrix of the top 10 missense variants by SAMPLE
missense_variants_matrix <- missense_variants_count %>% 
  spread(key = HGVS_P_1LETTER, value = n, fill = 0)
